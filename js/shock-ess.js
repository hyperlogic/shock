...
(function () {
    ...
    function uninitializedUpdate(dt) {
        ...
        var DEFAULT_SEARCH_RADIUS = 50;
        if (uninitializedUpdateCount % NUM_UPDATES_TO_SKIP === 0) {

            var foundEntityCount = 0;
            var i, l = entityNames.length;
            for (i = 0; i < l; i++) {
                print("AJT: Searching for " + entityNames[i]);
                var result = Entities.findEntitiesByName(entityNames[i], DEFAULT_CONTAINER_LOCATION, DEFAULT_SEARCH_RADIUS);
                print("AJT: found result = " + JSON.stringify(result));
                var entityID = result[0];
                if (entityID) {
                    entityIDMap[entityNames[i]] = entityID;
                    print("Found " + entityNames[i] + ": " + entityID);
                }
            }
    ...
    this.preload = function(entityID) {
        ...
        DEFAULT_CONTAINER_LOCATION = Entities.getEntityProperties(entityID, "position").position;
        print("AJT: entityID = " + entityID + ", position = " + JSON.stringify(DEFAULT_CONTAINER_LOCATION));
        ...
        Messages.messageReceived.connect(function (channel, message, senderID) {
            ...
            if (channel === TRIGGER_CHANNEL) {
                ...
                if (data.inside) {
                    if (data.entityID === lookupEntityByName("diefi-entry-back-trigger")) {
                        numAvatarsInEntryTriggerMap[senderID] = AVATAR_TRIGGER_TIMEOUT;
                    } else if (data.entityID === lookupEntityByName("diefi-container-trigger")) {
